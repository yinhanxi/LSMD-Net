...
class Aggregation(SubModule):
    def __init__(
        ...
        if gce:
            self.channelAttStem = channelAtt(8,
                                             2*in_chns[1]+spixel_branch_channels[1],
                                             #self.max_disp / 4,
                                             )
        ...
        for i in range(3):
            ...
            if gce:
                ...
                self.channelAttDown.append(
                    channelAtt(channels[i+1],
                               cdfeat_mul*in_chns[i+2],
                               #(self.max_disp / 4)//(2**(i+1)),
                    ))
    ...
    def forward(self, img, cost):
        ...
        cost = cost.reshape(b, -1, max_disp // 4, h, w)

...
class MSG_CHN(nn.Module):
    def __init__(self):
        super(MSG_CHN, self).__init__()
    ...
    def forward(self,input_rgb,input_d,train=False):
        C = (input_d > 0).float()
        ...
        '''feat = torch.cat((enc_d11[2],enc_c[2]),1)
        val = self.mlp(feat)
        val = F.interpolate(val,scale_factor=4,mode='bilinear',align_corners=True)
        val = self.activation(val)
        val = torch.clamp(val,0.01, 1.0)'''

        return output_d11, output_d12, output_d14 #,val
